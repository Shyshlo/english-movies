/*
Хотела сделать активной нажатую вкладку
...
//Анимация
gsap.to('.title',{
    text: "English with pleasure",
    duration: 3,  //продолжительность
    delay: 1, // задержка 2 сек после загрузки стр
})

gsap.from('.menu__item', {x: 400, duration: 1.5, rotation: 360, stagger: 0.5})
...
linkMoreVerbs.addEventListener('click', function() {
    ...
    linkMoreVerbs.style.display = "none";
})
...
function playArray() {
    ...
    const screenSong = document.querySelector('.screenSong');


    const songs = [
        {name: `Keane — everybody changing`,
        path: '/Music/mp3/Keane — Everybodys changing.mp3'},
        {name: `Lp — lost on you`,
        path: './Music/mp3/laura-pergolizzi-lost-on-you_(tetamix.org).mp3'},
        {name: `Queen - Bohemian rhapsody`,
        path: './Music/mp3/bohemian_rhapsody_07. Queen - Bohemian Rhapsody.mp3'},
        {name: `Queen - I want to break free`,
        path: './Music/mp3/muzlome_Queen_-_I_Want_To_Break_Free_47828518.mp3'},
        {name: `Feel - Robbie Williams`,
        path: '/Music/mp3/robbie_williams-feel.mp3'},
        {name: `Love Runs Out - One Republic`,
        path: '/Music/mp3/One Republic - Love Runs Out  (mp3store.cc).mp3'},
    ];

    const screen = ["./Music/screen/Keane.png", "./Music/screen/LP.png", "./Music/screen/Queen Bohemian.png", "./Music/screen/Queen I want.png", "./Music/screen/feel.png", "./Music/screen/one repablick.png"]
...
prev.addEventListener('click', () => {
    ...
    document.querySelector("#screen").src = screen[i];
    screenSong.style.display = 'block';
    audio.src = songs[i].path;
    par.textContent = songs[i].name;
